...
void iniciaJogo()
{
	char *bufferServidor = (char*)malloc(sizeof(char)*TAM_MSG);
	...
	while(aux != NULL)
	{
		...
		while(aux2 != NULL)
		{
			...
			serializaJogador(IncluiJogador, aux2, bufferServidor);
			send(aux->idSocket, bufferServidor , TAM_MSG, 0 );
			aux2 = aux2->prox;
		}
		printf("Setando o jogador %d \n",  aux->idJogador);
		serializaJogador(SetaJogador, aux,bufferServidor);
		send(aux->idSocket, bufferServidor , TAM_MSG, 0 );
