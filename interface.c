...
{
	bool moveu = false;
	if(pos[KEY_UP] && verifica_colisao(jogador->pos_x, jogador->pos_y, BOUNCER_SIZE, 0, velocidade))
	{
		if(pos[KEY_LEFT] || pos[KEY_RIGHT])
			jogador->pos_y -= velocidade*0.7;
		else
			jogador->pos_y -= velocidade;
		moveu = true;
		*refresh = true;
	}
 
	if(pos[KEY_DOWN] && verifica_colisao(jogador->pos_x, jogador->pos_y, BOUNCER_SIZE, 1, velocidade))
	{
		if(pos[KEY_LEFT] || pos[KEY_RIGHT])
			jogador->pos_y += velocidade*0.7;
		else
		...
		*refresh = true;
		moveu = true;
       	...
       	if(pos[KEY_LEFT] && verifica_colisao(jogador->pos_x, jogador->pos_y, BOUNCER_SIZE, 2, velocidade))
	{
		if(pos[KEY_UP] || pos[KEY_DOWN])
			jogador->pos_x -= velocidade*0.7;
		else
			jogador->pos_x -= velocidade;
		*refresh = true;
		moveu = true;
	}
 
	if(pos[KEY_RIGHT] && verifica_colisao(jogador->pos_x, jogador->pos_y, BOUNCER_SIZE, 3, velocidade))
	{
		if(pos[KEY_UP] || pos[KEY_DOWN])
			jogador->pos_x += velocidade*0.7;
		else
